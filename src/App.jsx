...
function App() {
    ...
    if (user) {
      ...
      const unsubConfig = onSnapshot(docRef, (docSnap) => {
        if (docSnap.exists()) {
          ...
          setDoc(docRef, { tecnicos: ['Técnico 1'], limites: { instalacao: 2, chamado: 2 }, tipos: [] });
        }
      });
  ...
  const checkCapacity = (techName, dateStr, turno, tipoServico, ignoreUid = null) => {
    ...
    const limiteAtual = categoriaAtual === 'instalacao' ? (limites?.instalacao || 2) : (limites?.chamado || 2);
  ...
  const handleDelete = async (uid) => { if (userRole !== 'admin') return alert("Apenas administradores podem excluir."); if(confirm("Excluir?")) await deleteDoc(doc(db, "os", uid)); };
    ...
    <div className="min-h-screen p-4 md:p-8 font-sans bg-[#f3f4f6] text-[#000000] flex flex-col">
      ...
      {currentTab === 'config' && (
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
          {userRole !== 'admin' && (
            <div className="col-span-full bg-orange-100 border-l-4 border-orange-500 text-orange-700 p-4 rounded-md flex items-center gap-2">
              <Lock size={20} />
              <div><p className="font-bold">Modo de Visualização</p><p className="text-xs">Apenas administradores podem alterar configurações.</p></div>
            </div>
          )}

          <div className="space-y-6">
             {/* EQUIPE */}
             <div className="bg-white p-6 rounded-xl shadow-sm border-t-4 border-[#EB6410]">
               <h2 className="text-xl font-bold mb-4 flex items-center gap-2"><User className="text-[#EB6410]" /> Equipe Técnica</h2>
               {userRole === 'admin' && (
                 <form onSubmit={handleAddTecnico} className="flex gap-2 mb-6"><input type="text" placeholder="Nome" className="flex-1 border-2 rounded-lg px-4 py-2" value={novoTecnico} onChange={e => setNovoTecnico(e.target.value)} /><button type="submit" className="bg-[#EB6410] text-white px-4 rounded-lg"><Plus/></button></form>
               )}
               <div className="space-y-2">
                 {(listaTecnicos || []).map((tec, idx) => (
                   <div key={idx} className="flex justify-between items-center p-3 bg-gray-50 rounded-lg border hover:border-[#EB6410]">
                     <span className="font-medium">{tec}</span>
                     {userRole === 'admin' && <button onClick={() => handleRemoveTecnico(tec)} className="text-gray-400 hover:text-red-500"><Trash2 size={18}/></button>}
                   </div>
                 ))}
               </div>
             </div>
             ...
             {/* USUÁRIOS */}
             {userRole === 'admin' && (
               <div className="bg-white p-6 rounded-xl shadow-sm border-t-4 border-gray-700">
                 <h2 className="text-xl font-bold mb-4 flex items-center gap-2"><Shield className="text-gray-700" /> Acesso de Usuários</h2>
                 <div className="space-y-2">
                   {(usuariosSistema || []).map((u) => (
                     <div key={u.uid} className="flex justify-between items-center p-3 bg-gray-50 rounded-lg border">
                       <div className="flex flex-col">
                         <span className="font-bold text-sm text-gray-800">{u.email}</span>
                         <span className={`text-xs font-bold uppercase ${u.role === 'admin' ? 'text-green-600' : 'text-gray-500'}`}>{u.role}</span>
                       </div>
                       {u.email !== user.email && (
                         <button onClick={() => handleToggleRole(u.uid, u.role)} className="text-xs bg-gray-200 hover:bg-gray-300 px-3 py-1 rounded font-bold transition">Mudar Cargo</button>
                       )}
                     </div>
                   ))}
          ...
          <div className="space-y-6">
            ...
            <div className="bg-white p-6 rounded-xl shadow-sm border-t-4 border-purple-500">
              <h2 className="text-xl font-bold mb-4 flex items-center gap-2"><Activity className="text-purple-500" /> Regras de Capacidade</h2>
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="block text-sm font-bold text-gray-600 mb-1">Max. Instalações</label>
                  <input type="number" min="0" disabled={userRole !== 'admin'} className="w-full border-2 rounded-lg px-3 py-2 disabled:bg-gray-100" value={limites?.instalacao || 0} onChange={e => handleUpdateLimites('instalacao', e.target.value)} />
                </div>
                <div>
                  <label className="block text-sm font-bold text-gray-600 mb-1">Max. Chamados</label>
                  <input type="number" min="0" disabled={userRole !== 'admin'} className="w-full border-2 rounded-lg px-3 py-2 disabled:bg-gray-100" value={limites?.chamado || 0} onChange={e => handleUpdateLimites('chamado', e.target.value)} />
                </div>
              </div>
            </div>
            ...
            <div className="bg-white p-6 rounded-xl shadow-sm border-t-4 border-blue-500">
              ...
              {userRole === 'admin' && (
                <form onSubmit={handleAddTipo} className="flex flex-col gap-2 mb-6"><input type="text" placeholder="Nome" className="w-full border-2 rounded-lg px-3 py-2 text-sm" value={novoTipoNome} onChange={e => setNovoTipoNome(e.target.value)} /><div className="flex gap-2"><input type="time" className="w-24 border-2 rounded-lg px-2 py-2 text-sm" value={novoTipoDuracao} onChange={e => setNovoTipoDuracao(e.target.value)} /><select className="flex-1 border-2 rounded-lg px-2 py-2 text-sm bg-white" value={novoTipoCategoria} onChange={e => setNovoTipoCategoria(e.target.value)}><option value="instalacao">Instalação</option><option value="chamado">Manutenção</option></select><button type="submit" className="bg-blue-500 text-white px-4 rounded-lg"><Plus/></button></div></form>
              )}
              <div className="space-y-2">
                {(listaTipos || []).map((t, idx) => (
                  <div key={idx} className="flex justify-between items-center p-3 bg-gray-50 rounded-lg border hover:border-blue-500">
                    <div className="flex flex-col">
                      ...
                      <div className="flex gap-2 text-xs text-gray-400"><span className="flex items-center gap-1"><Timer size={10}/> {t.duracao}h</span><span className="flex items-center gap-1"><Briefcase size={10}/> {t.categoria}</span></div>
                    </div>
                    {userRole === 'admin' && <button onClick={() => handleRemoveTipo(t.nome)} className="text-gray-400 hover:text-red-500"><Trash2 size={18}/></button>}
                  </div>
                ))}
              </div>
            </div>
