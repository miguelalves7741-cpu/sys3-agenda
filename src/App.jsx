...
import { 
  ...
  Timer, History, CalendarDays, GripHorizontal, Settings, Plus, Briefcase, Activity, Search, Shield, Lock, Trash2, MessageCircle, AlertTriangle, Sun, Moon, MapPin, Wrench, BarChart3, Coffee, CalendarOff, Bell
} from 'lucide-react';
...
import { UpdatesDrawer } from './components/SystemUpdates'; // <--- IMPORT DA SIDEBAR
...
function App() {
  ...
  // --- NOVIDADES ---
  const [showUpdates, setShowUpdates] = useState(false); // Estado para abrir a sidebar
      ...
      const unsubOS = onSnapshot(q, (snapshot) => {
        ...
        if (!isFirstLoad.current) {
          snapshot.docChanges().forEach((change) => {
            ...
            if (change.type === "added") setNotification({ type: 'success', title: 'Nova OS', message: `${data.tipo} - ${data.cliente.split(' ')[0]}`, user: autor });
            if (change.type === "modified") setNotification({ type: 'info', title: 'Atualização', message: `${data.cliente.split(' ')[0]}: ${data.status}`, user: autor });
            if (change.type === "removed") setNotification({ type: 'error', title: 'Exclusão', message: `OS removida.`, user: "Admin" });
          });
        }
  ...
  const handleSaveOS = async (e) => {
    ...
    try {
      ...
      const docRef = await addDoc(collection(db, "os"), { id: Date.now(), ...formData, status: 'Pendente', hora_inicio: '', hora_fim: '', last_editor: user.email });
  ...
  const handleUpdateStatus = async (uid, novoStatus) => { await updateDoc(doc(db, "os", uid), { status: novoStatus, last_editor: user.email }); await registrarHistorico(uid, "Status", novoStatus); };
  const handleUpdateField = async (uid, field, value, currentOS = null) => {
    ...
    let updateData = { [field]: value, last_editor: user.email };
    let acao = "Edição";
  ...
  const handleUpdateDate = async (uid, novaData, dataAntiga) => {
    ...
    await updateDoc(doc(db, "os", uid), { data: novaData, last_editor: user.email });
  ...
  const handleAddFolga = async (e) => {
    ...
    if (!folgaTecnico || !folgaData) return alert("Selecione técnico e data.");
    const exists = folgas.find(f => f.tecnico === folgaTecnico && f.data === folgaData);
    if (exists) return alert("Já existe uma regra para este técnico nesta data.");
    const novaFolga = { tecnico: folgaTecnico, data: folgaData, limite: Number(folgaLimite) };
    const novasFolgas = [...folgas, novaFolga];
    await updateDoc(doc(db, "configuracoes", "geral"), { folgas: novasFolgas });
    setFolgaTecnico(""); setFolgaData(""); setFolgaLimite(0);
  };

  const handleRemoveFolga = async (tecnico, data) => {
    if (userRole !== 'admin') return;
    if (confirm("Remover esta folga/regra?")) {
      const novasFolgas = folgas.filter(f => !(f.tecnico === tecnico && f.data === data));
      await updateDoc(doc(db, "configuracoes", "geral"), { folgas: novasFolgas });
    }
  };
           ...
           <div className="flex gap-4 min-w-full items-start">
              ...
              {(listaTecnicos || []).map(tec => {
                ...
                return (
                  <div 
                    key={tec} 
                    onDragOver={(e) => !isFolgaTotal && handleDragOverTech(e, tec)} 
                    onDrop={(e) => !isFolgaTotal && handleDropTech(e, tec)} 
                    className={`min-w-[300px] w-[300px] rounded-xl p-3 border-2 transition-colors ${
                      isFolgaTotal 
                        ? 'bg-red-50 border-red-200 opacity-90 cursor-not-allowed' 
                        : (dragOverTech === tec ? 'border-[#EB6410] bg-orange-50' : 'border-transparent bg-gray-100')
                    }`}
                  >
                    <div className="flex justify-between items-center mb-3 px-1">
                      <h3 className={`font-bold flex items-center gap-2 uppercase truncate ${isFolgaTotal ? 'text-red-600' : 'text-[#EB6410]'}`}>
                        <User size={16}/> {tec}
                        {isFolgaTotal && <span className="text-[10px] bg-red-100 text-red-600 px-2 py-0.5 rounded-full flex items-center gap-1"><CalendarOff size={10}/></span>}
                      </h3>
                      <span className={`${isFolgaTotal ? 'bg-red-200 text-red-800' : 'bg-orange-200 text-orange-800'} text-xs font-bold px-2 py-0.5 rounded-full`}>
                        {getTodayOS().filter(os => os.tecnico === tec).length}
                      </span>
                    </div>
                    
                    {/* CARD VISUAL DE ESCALA (APARECE SE EXISTIR QUALQUER REGRA) */}
                    {folgaHoje && (
                      <div className={`p-3 mb-3 rounded-lg border-l-4 shadow-sm flex items-center gap-3 animate-fade-in ${Number(folgaHoje.limite) === 0 ? 'bg-red-50 border-red-500' : 'bg-yellow-50 border-yellow-500'}`}>
                         <div className={`p-2 rounded-full ${Number(folgaHoje.limite) === 0 ? 'bg-red-100 text-red-600' : 'bg-yellow-100 text-yellow-600'}`}>
                            {Number(folgaHoje.limite) === 0 ? <Coffee size={20}/> : <AlertTriangle size={20}/>}
                         </div>
                         <div>
                            <h4 className={`text-xs font-bold uppercase ${Number(folgaHoje.limite) === 0 ? 'text-red-700' : 'text-yellow-700'}`}>
                              {Number(folgaHoje.limite) === 0 ? 'Folga Total' : 'Escala Ajustada'}
                            </h4>
                            <p className="text-[10px] text-gray-500 font-medium leading-tight">
                              {Number(folgaHoje.limite) === 0 ? 'Indisponível hoje.' : `Capacidade: ${folgaHoje.limite} vagas.`}
                            </p>
                         </div>
                      </div>
                    )}
